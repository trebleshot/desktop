...
DeviceChooserDialog::DeviceChooserDialog(QWidget *parent, groupid groupId)
        : QDialog(parent), m_groupId(groupId), m_ui(new Ui::DeviceChooserDialog), m_deviceModel(new NetworkDeviceModel)
{
    m_ui->setupUi(this);
...
void DeviceChooserDialog::selectionAccepted()
{
    ...
    for (const auto &modelIndex : selectionModel->selectedIndexes()) {
        ...
        devices << m_deviceModel->list()->at(modelIndex.row());
    }

    emit devicesSelected(devices, m_groupId);
}
